...
def parse_model(d, ch, nc, sync_bn=False):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d.backbone + d.head):  # from, number, module, args
        ...
        if m in (
            ...
            args = [c1, c2, *args[1:]]
            #print(f"outchannels for args {args}")
            if m in (
                ConvNormAct,
                RepConv,
                DownC,
            ...
            if m in (DownC, SPPCSPC, C3, C2f, DWC3, C3k2):
        ...
        elif m in (YOLOv8Head, YOLOv8SegHead, YOLOXHead, YOLOv11Head, YOLOv11SegHead):  # head of anchor free
            #print(f'Now in Head,\n ch is {ch} and length is {len(ch)}\n f is {f}')
            args.append([ch[x] for x in f])
            if m in (YOLOv8SegHead, YOLOv11SegHead):
