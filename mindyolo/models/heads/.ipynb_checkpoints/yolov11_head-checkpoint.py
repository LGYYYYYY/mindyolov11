...
from ..layers import DFL, ConvNormAct, Identity, DWConvNormAct
...
class YOLOv11Head(nn.Cell):
    ...
    max_det = 300
    shape = None
    #anchors = Tensor(shape=(0,), dtype=mstype.float32)
    #strides = Tensor(shape=(0,), dtype=mstype.float32)
    anchors = Tensor(0)
    strides = Tensor(0)
    legacy = False
    ...
    def __init__(self, nc=80, reg_max=16, stride=(), ch=(), sync_bn=False):  # detection layer
        ...
        # )
        self.cv3 = (
            nn.CellList([nn.SequentialCell([ConvNormAct(x, c3, 3, sync_bn=sync_bn),ConvNormAct(c3, c3, 3, sync_bn=sync_bn),nn.Conv2d(c3, self.nc, 1, has_bias=True),])for x in ch])
            if self.legacy
            else nn.CellList([
                nn.SequentialCell([
                    nn.SequentialCell([DWConvNormAct(x, x, 3, sync_bn=sync_bn), ConvNormAct(x, c3, 1, sync_bn=sync_bn)]),
                    nn.SequentialCell([DWConvNormAct(c3, c3, 3, sync_bn=sync_bn), ConvNormAct(c3, c3, 1, sync_bn=sync_bn)]),
                    nn.Conv2d(c3, self.nc, 1, has_bias=True),
                ])
                for x in ch
            ])
    ...
    def construct(self, x):
        ...
        # for i in range(self.nl):
        #     x[i] = ops.cat((self.cv2[i](x[i]), self.cv3[i](x[i])), 1)
        # if self.training:
        #     return x
        # y = self._inference(x)
        # return y if self.expor else (y,x)
    ...
    def initialize_biases(self):
        ...
        for a, b, s in zip(m.cv2, m.cv3, m.stride):  # from
            ...
            b_np[: m.nc] = math.log(5 / m.nc / (3072 / int(s)) ** 2)
            b[-1].bias = ops.assign(b[-1].bias, Tensor(b_np, ms.float32))
            # a[-1].bias.data[:] = 1.0
            # b[-1].bias.data[: m.nc] = math.log(5 / m.nc / (640/s) ** 2)
            
    def decode_bboxes(self, bboxes, anchors):
        return self.dist2bbox(bboxes, anchors, xywh=not self.end2end, axis=1)
            
    def _inference(self, x):
        shape = x[0].shape
        x_cat = ops.cat([xi.view(shape[0], self.no, -1) for xi in x], 2)
        if self.dynamic or self.shape != shape:
            self.anchors, self.strides = (x.transpore(0, 1) for x in self.make_anchors(x, self.stride, 0.5))
            self.shape = shape
        ...
        return ops.cat((dbox, cls.sigmoid()), 1)
        


class YOLOv11SegHead(YOLOv11Head):
