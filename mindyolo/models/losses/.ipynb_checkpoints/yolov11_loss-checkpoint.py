...
__all__ = ["YOLOv11Loss", "YOLOv11SegLoss"]
...
@register_model
class YOLOv11SegLoss(YOLOv11Loss):
    def __init__(self, box, cls, dfl, stride, nc, reg_max=16, nm=32, overlap=True, max_object_num=600, **kwargs):
        super(YOLOv11SegLoss, self).__init__(box, cls, dfl, stride, nc, reg_max)
...
class BboxLoss(nn.Cell):
    ...
    def construct(
        ...
        iou = bbox_inner_iou(pred_bboxes, target_bboxes, xywh=False, CIoU=True, ratio=0.75)
    ...
    if CIoU or DIoU or GIoU or EIoU or SIoU:
        ...
        if CIoU or DIoU or EIoU or SIoU:  # Distance or Complete IoU https://arxiv.org/abs/1911.08287v1
            ...
            if CIoU:  # https://github.com/Zzh-tju/DIoU-SSD-pytorch/blob/master/utils/box/box_utils.py#L47
                v = (4 / math.pi ** 2) * (ops.atan(w2 / h2) - ops.atan(w1 / h1)).pow(2)
                #with torch.no_grad():
                alpha = v / (v - iou + (1 + eps))
                ...
                sigma = ops.pow(s_cw ** 2 + s_ch ** 2, 0.5)
                sin_alpha_1 = ops.abs(s_cw) / sigma
                sin_alpha_2 = ops.abs(s_ch) / sigma
                threshold = pow(2, 0.5) / 2
                sin_alpha = ops.where(sin_alpha_1 > threshold, sin_alpha_2, sin_alpha_1)
                angle_cost = ops.cos(ops.arcsin(sin_alpha) * 2 - math.pi / 2)
                rho_x = (s_cw / cw) ** 2
                rho_y = (s_ch / ch) ** 2
                gamma = angle_cost - 2
                distance_cost = 2 - ops.exp(gamma * rho_x) - ops.exp(gamma * rho_y)
                omiga_w = ops.abs(w1 - w2) / ops.max(w1, w2)
                omiga_h = ops.abs(h1 - h2) / ops.max(h1, h2)
                shape_cost = ops.pow(1 - ops.exp(-1 * omiga_w), 4) + ops.pow(1 - ops.exp(-1 * omiga_h), 4)
